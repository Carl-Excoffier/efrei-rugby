function loadDashboard() {
    fetch("api/user")
        ...
        .then((data) => {
            if (data.success) {
                ...
                // Guard constraint against element duplication in the navigation list during layout cycles
                if (
    ...
    fetch("api/next-training")
        ...
        .then((data) => {
            if (data.success) {
                const nextTraining = document.getElementById("next-training");
                if (data.training) {
                    const date = new Date(data.training.date);
                    nextTraining.textContent = date.toLocaleDateString(
                        "en-GB",
                        {
                            weekday: "long",
                            day: "numeric",
    ...
    fetch("api/latest-photos")
        ...
        .then((data) => {
            if (data.success && data.photos.length > 0) {
                ...
                // Reconstructs public-facing paths bound to isolated backend file storage keys
    ...
    fetch("api/latest-review")
        ...
        .then((data) => {
            ...
            if (data.success && data.review) {
                ...
                // Forces string evaluation into uniform decimal positioning format (e.g. 4.0 / 5.0)
                ratingP.append(
                    "⭐ ",
                    strongTag,
